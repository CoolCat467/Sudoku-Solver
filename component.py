...
from typing import Optional, Any, overload, TypeVar
...
class Event(dict[str, T]):
    ...
    @overload
    def __init__(self, name: str, data: Iterable[tuple[str, T]] | None = None, /, **kwargs: None) -> None: ...
    @overload
...
class Component:
    ...
    def __manager_ref_dead(self, dead_ref: 'ComponentManager') -> None:
        ...
        # unbind subclasses could potentially forget to call super()
        self.__manager = None
        self.unbind()
    ...
    def remove_handler(self,
                       event_name: str) -> Handler | None:
        "Unregister an event handler and return it if it exists, else None"
        if event_name in self.__handlers:
...
class ComponentManager(Component):
    ...
    def add_component(self, component: Component) -> None:
        ...
        if not isinstance(component, Component):
            raise ValueError('component argument is not a Component instance!')
        if component.name in self.__components:
            raise NameError(f'Component named "{component.name}" already exists!')
        ...
        for name in component.get_handled():
            if name not in self.handled_events:
                ...
                if name not in self.get_handled():
                    ...
                    if current is None:
                        raise ValueError(f'Registered handler "{name}" cannot not-exist if registered!')
