...
from typing import Union, cast, TypeVar, overload
...
class Location(Vector2[L]):
    "Location bound to the center of a given rectangle"
    __slots__ = ('_rect',)
    @overload
    def __new__(cls, *args: L, dtype: type) -> Vector2[L]: ...  # type: ignore[misc]
    @overload
    def __new__(cls, *args: L, dtype: None=None) -> 'Location[L]': ...
    def __new__(cls,# type: ignore[misc]
                *args: L,
                dtype: type | None=None) -> 'Location[L] | Vector2[L]':
    ...
    def __init__(self, rect: Rect) -> None:
        ...
        self._rect = rect
        super().__init__(*cast(tuple[L, L], self._rect.center), dtype=list)
