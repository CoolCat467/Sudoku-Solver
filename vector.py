...
class Vector(Iterable[V]):
    ...
    def __init__(self, *args: V, dims: int | None = None, dtype: type = tuple) -> None:
        ...
        if dims is None:
            ...
            args = args[:dims]
            self.__v = cast(
                Iterable[V], self.dtype(list(args) + [0] * (dims - len(args)))
            )
    ...
    @classmethod
    def from_points(
        cls, point1: Union[Iterable[T], T], point2: Iterable[T]
    ) -> "Vector[T]":
    ...
    def __round__(self, ndigits: int | None = None) -> "Vector[int]":
        ...
        return self.__class__.from_iter(
            (round(x, ndigits) for x in self.__v), dtype=self.dtype
        )
...
class Vector1(Vector[V]):
    ...
    @classmethod
    def from_iter(
        cls, iterable: Iterable[T], dims: int | None = None, dtype: type = list
    ) -> "Vector1[T]":
